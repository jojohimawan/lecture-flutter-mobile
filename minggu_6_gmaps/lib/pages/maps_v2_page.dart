...
class _MapsV2PageState extends State<MapsV2Page> {
  ...
  Future searchLocation() async {
    try {
      var value = await GeocodingPlatform.instance!.locationFromAddress(address);
      GoogleMapController controller = await _controller.future;
      print(value);
      ...
      //   controller.animateCamera(cameraUpdate);
      //   print("HITTED: ${value}" );
      //   Fluttertoast.showToast(msg: "bisa sih harusnya");
      // });
    } catch (error) {
      Fluttertoast.showToast(msg: "Lokasi tidak ditemukan");
      print(error);
    } finally {
      Fluttertoast.showToast(msg: "Lokasi ditemukan");
    }
  }
}